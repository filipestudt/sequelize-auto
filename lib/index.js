  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      if (!self.options.typescript) {
        ...
        text[table] = "const { Model, DataTypes, ValidationError } = require('sequelize');\n\n";
        text[table] += "class " + tableName.charAt(0).toUpperCase() + tableName.slice(1) + " extends Model {\n";
        text[table] += spaces + "static init(sequelize) {\n";
        text[table] += spaces + spaces + "super.init({\n";
      ...
      _.each(fields, function (field, i) {
        ...
        text[table] += spaces + spaces + spaces + fieldName + ": {\n";
        ...
        _.each(fieldAttr, function (attr, x) {
          ...
          if (attr === "foreignKey") {
            if (isSerialKey) {
              text[table] += ",\n" + spaces + spaces + spaces + spaces + "autoIncrement: true";
            }
            else if (foreignKey.isForeignKey) {
              text[table] += ",\n" + spaces + spaces + spaces + spaces + "references: {\n";
              text[table] += spaces + spaces + spaces + spaces + spaces + "model: \'" + self.tables[table][field][attr].foreignSources.target_table + "\',\n"
              text[table] += spaces + spaces + spaces + spaces + spaces + "key: \'" + self.tables[table][field][attr].foreignSources.target_column + "\'\n"
              text[table] += spaces + spaces + spaces + spaces + "}";

              associations.push({
                field: field,
                model: self.tables[table][field][attr].foreignSources.target_table,
                column: self.tables[table][field][attr].foreignSources.target_column
              })
            ...
            if (self.tables[table][field][attr] === true && (!_.has(self.tables[table][field], 'foreignKey') || (_.has(self.tables[table][field], 'foreignKey') && !!self.tables[table][field].foreignKey.isPrimaryKey))) {
              text[table] += ",\n" + spaces + spaces + spaces + spaces + "primaryKey: true,\n";
              text[table] += spaces + spaces + spaces + spaces + "update: false";

              if (self.tables[table][field].type.toLowerCase() == 'character varying') {
                text[table] += ",\n" + spaces + spaces + spaces + spaces + "nanoid: true";
              }
            ...
            // Only sets allowNull if it's false
            // Also create a notEmpty validation that is trigger because of the defaultValue=''
            if (self.tables[table][field][attr] == false) {
              text[table] += ",\n" + spaces + spaces + spaces + spaces + attr + ": " + self.tables[table][field][attr];
              text[table] += ",\n" + spaces + spaces + spaces + spaces + "defaltValue: ''";
              text[table] += ",\n" + spaces + spaces + spaces + spaces + "validate: {";
              text[table] += "\n" + spaces + spaces + spaces + spaces + spaces + "notEmpty: {";
              text[table] += "\n" + spaces + spaces + spaces + spaces + spaces + spaces + `msg: 'O campo "${field}" é obrigatório'`;
            ...
            if (defaultVal === null || defaultVal === undefined) {
              ...
              text[table] += ",\n" + spaces + spaces + spaces + spaces + attr + ": " + val_text;
            }
          }
          else if (attr === "type" && self.tables[table][field][attr].indexOf('ENUM') === 0) {
            text[table] += spaces + spaces + spaces + spaces + attr + ": DataTypes." + self.tables[table][field][attr];
          } else {
            ...
            text[table] += spaces + spaces + spaces + spaces + attr + ": " + val;
        ...
        if (self.options.camelCase) {
          text[table] += spaces + spaces + spaces + spaces + "field: '" + field + "',\n";
        }
      ...
      text[table] += spaces + spaces + spaces + "tableName: '" + table + "',\n";
      text[table] += spaces + spaces + spaces + "sequelize,\n";
      ...
      function addAdditionalOption(value, key) {
        if (key === 'name') {
          ...
          //text[table] += spaces + spaces + spaces + key + ": " + value + ",\n";
        }
      }
      ...
      text[table] += spaces + "static associate(models) {";
      ...
      for (let association of associations) {
        ...
        text[table] += "\n" + spaces + spaces + "this.belongsTo(models.";
        text[table] += association.model.charAt(0).toUpperCase() + association.model.slice(1);
        text[table] += ", {\n" + spaces;
        text[table] += spaces + spaces + "foreignKey: " + "'" + association.field + "',\n";
        text[table] += spaces + spaces + spaces + "as: '" + association.model + "'\n";

        text[table] += spaces + spaces + "});";

        //text[table] += spaces + spaces + "model: " + association.model;
        //text[table] += spaces + spaces + "column: " + association.column;
      }
...
AutoSequelize.prototype.write = function (attributes, typescriptFiles, callback) {
  ...
  function createFile(table, _callback) {
    ...
    fs.writeFile(path.resolve(path.join(self.options.directory, fileName.charAt(0).toUpperCase() + fileName.slice(1) + (self.options.typescript ? '.ts' : '.js'))), attributes[table], _callback);
  }
};
